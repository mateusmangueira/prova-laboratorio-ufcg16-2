...
public class IpirangaImpostoSimplesBRTest {
	...
	public IpirangaImpostoSimplesBRTest() throws Exception {
		...
		ipiranga.adicionarCaminhoneiro("Renato Rocha", rrCaminhoneiro, 15000,
				200);
		ipiranga.adicionarCaminhoneiro("Renato Rocha Jr", rrJrCaminhoneiro, 0,
				10);
		ipiranga.adicionarProfessor("Francisco Feij�o", ffProfessor, 5000,
				100000);
		ipiranga.adicionarProfessor("Francisco Feij�o Jr", ffJrProfessor, 0,
				880);
		ipiranga.adicionarProfessor("Francisco Feij�o Me", ffMeProfessor, 0,
				4400);
	}
	...
	private void testeNomesEBens() throws Exception {
		Assert.assertEquals("[TAXISTA] Pedro Pedreiro, 111.111.111-11.",
				ipiranga.getString(ppTaxista));
		Assert.assertEquals("[TAXISTA] Pedro Pedreiro Jr, 222.222.222-22.",
				ipiranga.getString(ppJrTaxista));
		Assert.assertEquals("[CAMINHONEIRO] Renato Rocha, 333.333.333-33.",
				ipiranga.getString(rrCaminhoneiro));
		Assert.assertEquals("[CAMINHONEIRO] Renato Rocha Jr, 444.444.444-44.",
				ipiranga.getString(rrJrCaminhoneiro));
		Assert.assertEquals("[PROFESSOR] Francisco Feij�o, 555.555.555-55.",
				ipiranga.getString(ffProfessor));
		Assert.assertEquals("[PROFESSOR] Francisco Feij�o Jr, 666.666.666-66.",
				ipiranga.getString(ffJrProfessor));
	...
	private void testeOrdemImposto() throws Exception {
		List<String> ordenadosPorImposto = ipiranga
				.getContribuintesOrdenadosPorImposto();
		String[] expected = { ppJrTaxista, ffJrProfessor, ffMeProfessor,
				rrJrCaminhoneiro, ppTaxista, ffProfessor, rrCaminhoneiro };
	...
	private void testeExtraException() {
		try {
			...
			Assert.fail("Era esperado uma exce��o.");
		} catch (Exception e) {
		...
		try {
			...
			Assert.assertEquals("CPF inv�lido.", e.getMessage());
		}
		try {
